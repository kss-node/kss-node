...
describe('KssBuilderNunjucks builder', function() {
  before(function() {
    ...
    return cli({
      stdout: stdout,
      stderr: stderr,
      argv: ['node', 'bin/kss', 'test/fixtures/with-include', 'test/output/nunjucks', '--builder', 'builder/nunjucks', '--title', 'KssBuilderNunjucks Test Style Guide', '--verbose']
    }).then(() => {
      ...
      return Promise.all(
        [
          ...
          return fs.readFileAsync(path.join(__dirname, 'output', 'nunjucks', fileName + '.html'), 'utf8').then(data => {
  ...
  it('should render the --title option', function() {
    expect(this.files['index']).to.include('<title>KssBuilderNunjucks Test Style Guide</title>');
  });
});
