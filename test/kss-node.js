...
suite('Command Line Interface', function() {
  ...
  suite('Option: --source', function() {
    test('Should read source directory from option', function(done) {
      exec('bin/kss-node --verbose --source test/fixtures-styles/with-include --destination test/output/nested', function(err, stdout) {
        assert.ifError(err);
    ...
    test('Should not declare success if source directory is empty', function(done) {
      exec('bin/kss-node --verbose --source test/fixtures-styles/empty-source --destination test/output/nested', function(err, stdout) {
    ...
    test('Warn if homepage content is not found', function(done) {
      exec('bin/kss-node --source test/fixtures-styles/missing-homepage --destination test/output/nested', function(err, stdout) {
        assert.ifError(err);
    ...
    test('Should read multiple source directories from option', function(done) {
      exec('bin/kss-node --verbose --source test/fixtures-styles/with-include --source test/fixtures-styles/empty-source --destination test/output/nested', function(err, stdout) {
        assert.ifError(err);
  ...
  suite('Option: --destination', function() {
    test('Should read destination directory from option', function(done) {
      exec('bin/kss-node --verbose test/fixtures-styles/with-include --destination test/output/nested', function(err, stdout) {
        assert.ifError(err);
        assert.ok(/\* Destination : .+test\/output\/nested/g.test(stdout), 'Read --destination option');
        done();
      });
    });
  });
  ...
  suite('Option: --custom', function() {
    test('Should read custom properties from option', function(done) {
      exec('rm -r test/output; bin/kss-node test/fixtures-styles/with-include test/output/nested --template test/fixtures-styles/template --custom custom --custom custom2; cat test/output/nested/section-4.html', function(err, stdout) {
        assert.ifError(err);
  ...
  suite('Option: --config', function() {
    test('Should load configuration from json file', function(done) {
      exec('bin/kss-node --config test/cli-option-config.json', function(err, stdout) {
        assert.ifError(err);
  ...
  suite('Handlebars helper', function() {
    ...
    test('Should load additional Handlerbars helpers with --helpers option', function(done) {
      exec('rm -r test/output; bin/kss-node test/fixtures-styles/with-include test/output/nested --template test/fixtures-styles/template --helpers test/fixtures-styles/template/helpers; cat test/output/nested/index.html', function(err, stdout) {
        assert.ifError(err);
    ...
    test('Should load Handlerbars helper: {{section [arg]}}', function(done) {
      exec('cat test/output/nested/section-3.html', function(err, stdout) {
        assert.ifError(err);
    ...
    test('Should load Handlerbars helper: {{eachSection [arg]}}', function(done) {
      exec('cat test/output/nested/section-2.html', function(err, stdout) {
        assert.ifError(err);
    ...
    test('Should load Handlerbars helper: {{eachRoot}}', function(done) {
      exec('cat test/output/nested/index.html', function(err, stdout) {
        assert.ifError(err);
