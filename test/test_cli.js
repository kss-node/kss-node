...
describe('Command Line Interface', function() {
  ...
  describe('given no arguments', function() {
    it('should display help', function() {
      return kssCLI('').then(function(result) {
  ...
  describe('given unnamed option', function() {
    it('should use the first unnamed as the source directory', function() {
      ...
      return kssCLI('--verbose ' + source + ' --destination ' + destination).then(function(result) {
        expect(result.error).to.not.exist;
    ...
    it('should use the second unnamed as the destination directory', function() {
      let source = helperUtils.fixtures('with-include'),
        destination = helperUtils.fixtures('..', 'output', 'nested');
      return kssCLI('--verbose ' + source + ' ' + destination).then(function(result) {
        expect(result.error).to.not.exist;
  ...
  describe('given --config option', function() {
    it('should load options from a JSON file', function() {
      return kssCLI('--config test/fixtures/cli-option-config-source-array.json').then(function(result) {
        expect(result.error).to.not.exist;
        expect(result.stdout).to.include(successMessage);
      });
    });

    it('should load the Twig builder when the config file uses "builder/twig"', function() {
      return kssCLI('--config test/fixtures/cli-option-config-twig.json --verbose').then(result => {
        expect(result.error).to.not.exist;
        expect(result.stdout).to.include(successMessage);
    ...
    it('should catch an error if the builder path is not a directory', function() {
      return kssCLI('--config test/fixtures/cli-option-config-builder-notdirectory.json --verbose').then(result => {
        expect(result.error).to.exist;
    ...
    it('should add the first unnamed as the source directory', function() {
      ...
      return kssCLI('--config test/fixtures/cli-option-config.json --verbose ' + source).then(function(result) {
        expect(result.error).to.not.exist;
        ...
        return kssCLI('--config test/fixtures/cli-option-config-source-array.json --verbose ' + source2).then(function(result) {
          expect(result.error).to.not.exist;
  ...
  describe('given --builder option', function() {
    it('should catch a failure when the builder API is not equal to the current API', function() {
      return kssCLI('--builder ' + helperUtils.fixtures('old-builder')).then(result => {
        expect(result.error).to.exist;
    ...
    it('should load the Twig builder when given "--builder builder/twig"', function() {
      return kssCLI('--builder builder/twig --verbose --source ' + helperUtils.fixtures('source-twig-builder-test') + ' --destination ' + helperUtils.fixtures('..', 'output', 'nested')).then(result => {
        expect(result.error).to.not.exist;
        expect(result.stdout).to.include(successMessage);
        expect(result.stdout).to.include('Builder     : ' + path.resolve(__dirname, '..', 'builder', 'twig'));
      });
  ...
  describe('given --verbose option two or more times', function() {
    it('should display error in builder with stack', function() {
      return kssCLI('--verbose --verbose --builder ' + helperUtils.fixtures('old-builder')).then(response => {
    ...
    it('should display error in kss() with stack', function() {
      return kssCLI('--verbose --verbose').then(function(response) {
