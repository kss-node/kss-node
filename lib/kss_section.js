...
KssSection = function(data) {
  ...
  // Find and add custom properties.
  for (name in data) {
    if (data.hasOwnProperty(name) && !this.meta.hasOwnProperty(name) && !this.data.hasOwnProperty(name)) {
...
KssSection.prototype.custom = function(name, value) {
  ...
  // @TODO: custom property names should be tracked in the KssStyleGuide.
