 ...
 *
 * ```
 * kss.traverse('./stylesheets', { mask: '*.css' }, function(error, styleGuide) {
 ...
 */
let traverse = function(directory, options, callback) {
  options = options || {};
  ...
  let walk = function(directories, opts, cb) {
    opts = opts || {};
    ...
    directories.forEach(function(dir) {
      ...
      filesRemaining += 1;
      fs.readdir(dir, function(error, relnames) {
        if (error) {
          cb(error);
          return;
        }
        ...
        relnames.forEach(relname => {
          loopsRemaining -= 1;
          let name = path.join(dir, relname);
  ...
  // parse their output.
  walk(directory, options, function() {
    /* eslint-disable no-loop-func */
    fileNames.sort();
    let numFileNames = fileNames.length,
      files = [],
      orderedObject = {};
    for (let i = 0; i < numFileNames; i += 1) {
      (function(j) {
        fs.readFile(fileNames[j], 'utf8', function(error, contents) {
          // istanbul ignore if
