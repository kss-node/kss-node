...
const kss = function(options) {
  ...
  return KssBuilderBase.loadBuilder(options.builder || require('../builder/handlebars')).then(newBuilder => {
    builder = newBuilder;
    ...
    // Show the full error stack if the verbose option is 2 or more.
    if (builder && builder.getOptions && builder.getOptions('verbose') > 1) {
      options.pipes.stderr.write((error.stack ? error.stack : /* istanbul ignore next */ error) + '\n');
    } else {
