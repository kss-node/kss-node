...
const kss = function(options) {
  ...
  return KssBuilderBase.loadBuilder(options.builder || require('../builder/handlebars')).catch(error => {
    ...
    // @TODO: Check 'source' array for paths ending in .json and convert it into
    // a KssStyleGuide object.

    // Then traverse the source and parse the files found.
    let traverseTask = traverse(builder.getOptions('source'), {
      header: true,
      markdown: true,
      markup: true,
    ...
    if (builder.getOptions('json')) {
      return traverseTask.then(styleGuide => {
