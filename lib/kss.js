...
var walk = require('./walk.js'),
	...
	traverse, parse, parseChunk, checkReference, findBlocks, processSection,
...
parseChunk = function(data, input, options) {
	...
	for (i = 0; i < l; i += 1) {
		...
		// Before anything else, process the sections that are clearly labeled and
		// can be found right away and then removed.
		currSection = processSection('Markup', paragraphs, options, currSection);
		...
		// Set the depth of the reference.
		currSection.depth = currSection.reference.split(/(?:\.|\s+\-\s+)/).length;
...
checkReference = function(paragraphs, options) {
	...
	// Search for the "styleguide" keyword at the start of the paragraph.
	if (words[0].replace(/\:?$/, '').toLowerCase() === 'styleguide') {
		keyword = words.shift();
	}
	else if (options.typos) {
		// Search for a one or two word typo of "styleguide".
		[words[0], words[0] + words[1]].forEach(function(value, index) {
			if (!keyword && natural.Metaphone.compare('Styleguide', value.replace(/(-|:$)/, ''))) {
				keyword = words.shift();
				if (index === 1) {
					keyword += ' ' + words.shift();
				}
...
/**
 * Checks if there is a specific section in the comment block and removes it from the original array.
 ...
 */
processSection = function(sectionName, paragraphs, options, sectionData, processValue) {
	var indexToRemove = 'not found',
		sectionName = sectionName.toLowerCase();

	paragraphs.map(function(paragraph, index) {
		if (hasPrefix(paragraph, options, sectionName)) {
			sectionData[sectionName] = paragraph.replace(/^\s*[a-z ]+\:\s+?/i, '');
			if (typeof processValue == "function") {
				sectionData[sectionName] = processValue(sectionData[sectionName]);
			}
			paragraph = '';
			indexToRemove = index;
		}
