...
const kss = function(options) {
  ...
  let builder;
  try {
    builder = require(path.resolve(kssConfig.getConfig('builder')));
  } catch (error) {
  ...
  return KssBuilder.checkBuilder(builder).then(() => {
    ...
    if (kssConfig.getConfig('clone')) {
      builder.log('Creating a new style guide builder in ' + kssConfig.getConfig('clone') + '...');

      return builder.clone(kssConfig.getConfig('builder'), kssConfig.getConfig('clone')).then(() => {
    ...
    if (!kssConfig.getConfig('source').length) {
      return Promise.reject(new Error('No "source" option specified.'));
    }

    // Initialize the builder.
    return builder.init(kssConfig.getConfig()).then(() => {
      if (kssConfig.getConfig('verbose')) {
        builder.log('...Parsing your style guide:');
      }

      // Then traverse the source and parse the files found.
      return traverse(kssConfig.getConfig('source'), {
        header: true,
        markdown: true,
        markup: true,
        mask: kssConfig.getConfig('mask'),
        custom: kssConfig.getConfig('custom')
      });
    }).then(styleGuide => {
