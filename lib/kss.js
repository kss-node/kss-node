...
const kss = function(options) {
  ...
  return new Promise((resolve, reject) => {
    ...
    generator.checkGenerator(error => {
      if (error) {
        return reject(error);
      } else {
        resolve();
    ...
    if (kssConfig.get('clone')) {
      ...
      return new Promise((resolve, reject) => {
        generator.clone(kssConfig.get('template'), kssConfig.get('clone'), error => {
          if (error) {
            reject(error);
          } else {
    ...
    return new Promise((resolve, reject) => {
      ...
      generator.init(kssConfig.get(), error => {
        if (error) {
          reject(error);
        } else {
          resolve();
      ...
      return new Promise((resolve, reject) => {
        ...
        generator.prepare(styleGuide, (error, styleGuide) => {
          if (error) {
            reject(error);
          } else {
            resolve(styleGuide);
      ...
      return new Promise((resolve, reject) => {
        ...
        generator.generate(styleGuide, error => {
          if (error) {
            reject(error);
          } else {
            resolve();
