...
let parse, parseChunk,
  createModifiers, createParameters,
  checkReference, processProperty, hasPrefix;

/**
 * Parse an array/string of documented CSS, or an object of files and their
 * content.
 *
...
parse = function(input, options) {
  ...
  // If supplied an array of strings or objects.
    ...
    files = input.map(value => {
      return (typeof value === 'string') ? {contents: value} : value;
    });
  ...
  for (let file of files) {
    styleGuide = parseChunk(styleGuide, file.contents, options);
  }
...
parseChunk = function(styleGuide, input, options) {
  ...
  let comments = findCommentBlocks(input);
  ...
  for (let comment of comments) {
    ...
    processProperty.call(currSection, paragraphs, options, 'Markup');
    processProperty.call(currSection, paragraphs, options, 'Weight', function(value) {
      return isNaN(value) ? 0 : parseFloat(value);
    });
    // Process custom properties.
    options.custom.forEach(name => {
      processProperty.call(currSection, paragraphs, options, name);
    });
 ...
 *   inserted into `this`.
 */
processProperty = function(paragraphs, options, propertyName, processValue) {
