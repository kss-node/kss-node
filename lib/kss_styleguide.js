...
class KssStyleGuide {
  ...
  toJSON() {
    ...
    returnObject = {
      ...
      hasNumericReferences: this.meta.hasNumericReferences,
      referenceDelimiter: this.meta.referenceDelimiter
    };
  ...
  init() {
    if (this.data.sections.length) {
      ...
      if (this.meta.needsSort) {
        let delimiter = this.meta.referenceDelimiter;
  ...
  sections(sections) {
    ...
    if (typeof sections === 'object' && !(sections instanceof RegExp)) {
      ...
      sections.forEach(section => {
        let originalDelimiter = this.meta.referenceDelimiter;
        ...
        if (originalDelimiter !== this.meta.referenceDelimiter) {
          this.meta.needsDepth = true;
        } else {
          // Set the depth of this section's reference.
          section.depth(section.reference().split(this.meta.referenceDelimiter).length);
        }
    ...
    if (!(query instanceof RegExp)) {
      let delim = this.meta.referenceDelimiter === '.' ? '\\.' : '\\ \\-\\ ';
      query = new RegExp(
        query
