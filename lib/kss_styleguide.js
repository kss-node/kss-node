...
KssStyleGuide = function(data) {
  ...
  this.meta = {
    ...
    files: data.files || [],
    referenceDelimiter: '.',
    referenceMap: data.section_refs || [],
    weightMap: false
  };
...
KssStyleGuide.prototype.init = function() {
  ...
  if (this.data.sections.length) {
    ...
    for (i = 0; i < l; i += 1) {
      ...
      this.data.sections[i].autoincrement(autoIncrement.join('.'));
      previousRef = ref;
    }
  }
};
...
KssStyleGuide.prototype.customPropertyNames = function(names) {
  ...
  for (var i = 0; i < names.length; i++) {
    if (this.meta.customPropertyNames.indexOf(names[i]) === -1) {
      this.meta.customPropertyNames.push(names[i]);
    }
  }
  // Allow chaining.
  return this;
