 ...
 */
KssStyleGuide.prototype.toJSON = function() {
  var returnObject;
  ...
  returnObject.sections = this.sections().map(function(section) {
 ...
 *   chaining of methods.
 */
KssStyleGuide.prototype.autoInit = function(autoInit) {
  this.meta.autoInit = autoInit ? true : false;
...
KssStyleGuide.prototype.init = function() {
  var self = this, i, numSections, getWeight, incrementIndex, autoIncrement = [0], ref, previousRef = [], index;

  if (this.data.sections.length) {
    numSections = this.data.sections.length;
    ...
    if (this.meta.needsSort) {
      ...
      // for section 4.3.
      getWeight = function(reference, depth) {
        reference = reference.toLowerCase();
        reference = reference.split(self.meta.referenceDelimiter, depth).join(self.meta.referenceDelimiter);

        return self.meta.weightMap[reference] ? self.meta.weightMap[reference] : 0;
      };
      ...
      this.data.sections.sort(function(a, b) {
        ...
        var refsA = a.reference().toLowerCase().split(self.meta.referenceDelimiter),
          ...
          weightA, weightB,
          j, l = Math.max(refsA.length, refsB.length);
        ...
        for (j = 0; j < l; j++) {
          if (refsA[j] && refsB[j]) {
            ...
            if (refsA[j] !== refsB[j]) {
              ...
              weightA = getWeight(a.reference(), j + 1);
              weightB = getWeight(b.reference(), j + 1);
              if (weightA !== weightB) {
                return weightA - weightB;
              } else if (refsA[j].match(/^\d+$/) && refsB[j].match(/^\d+$/)) {
                // If both chunks are digits, use numeric sorting.
                return refsA[j] - refsB[j];
              } else {
                // Otherwise, use alphabetical string sorting.
                return (refsA[j] > refsB[j]) ? 1 : -1;
              }
            }
          } else {
 ...
 * @returns {KssStyleGuide|KssSection|KssSection[]|false} If `sections` is
...
KssStyleGuide.prototype.sections = function(sections) {
  ...
  if (typeof sections === 'object' && !(sections instanceof RegExp)) {
    ...
    sections.forEach(function(section) {
      var originalDelimiter = self.meta.referenceDelimiter;
      ...
      self.meta.weightMap[section.reference().toLowerCase()] = section.weight();
      // Determine the separator used in references; e.g. 'a - b' or 'a.b'.
      if (section.reference().indexOf(' - ') > -1) {
        self.meta.referenceDelimiter = ' - ';
      }
      ...
      if (originalDelimiter !== self.meta.referenceDelimiter) {
        self.meta.needsDepth = true;
      } else {
        // Set the depth of this section's reference.
        section.depth(section.reference().split(self.meta.referenceDelimiter).length);
      }

      // Determine if all sections need their reference number recalculated.
      if (!self.meta.hasNumericReferences) {
        self.meta.needsReferenceNumber = true;
      }

      // A new section means all sections need to be sorted.
      self.meta.needsSort = true;
    });
  ...
  for (i = 0; i < this.data.sections.length; i += 1) {
    match = this.data.sections[i].reference().match(query);
