...
const cli = function(opts) {
  ...
  let settings = yargs(args).options(
    // We merge the CLI options with the default KssBuilder options.
    (new KssBuilder()).addOptions(cliOptions).getOptions()
  ).argv;
  ...
  // Set up an error handler for errors that handle in this module.
  ...
  return KssBuilder.checkBuilder(builder).catch(cliError).then(() => {
    // istanbul ignore else
    builder.addOptions(cliOptions);
    ...
    for (let key in configFile) {
      if (configFile.hasOwnProperty(key) && builder.getOptions()[key] && builder.getOptions()[key].path) {
