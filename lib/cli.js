...
const cli = function(opts) {
  ...
  if (args.indexOf('--demo') !== -1) {
    ...
    reportMessage('WELCOME to the kss-node demo! We\'ve turned on the --verbose flag so you can see what kss-node is doing.');
  }
  ...
  return KssBuilderBase.loadBuilder(options.builder).catch(error => {
    reportError(error);
