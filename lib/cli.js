...
const cli = function(opts) {
  ...
  let settings = yargs(args).options(
    // We merge the CLI options with the default KssBuilderBase options.
    (new KssBuilderBase()).addOptions(cliOptions).getOptions()
  ).argv;
  ...
  return KssBuilderBase.checkBuilder(builder).catch(cliError).then(() => {
