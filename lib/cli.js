...
const cli = function(opts, done) {
  ...
  // requested template's generator so that kssConfig will load the options of
  // the generator and of the template.
  kssConfig.set({template: settings.template});
  kssConfig.loadGenerator(function(error) {
    if (error) {
