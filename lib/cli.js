  ...
  const cliOptionDefinitions = {
    'config': {
      ...
      configParser: (configPath) => {
        ...
        config.homepage = path.resolve(path.dirname(configPath), config.homepage || 'homepage.md');

        if (config.source) {
          if (Array.isArray(config.source)) {
            config.source = config.source.map(x => path.resolve(path.dirname(configPath), x));
          } else {
            config.source = path.resolve(path.dirname(configPath), config.source);
          }
        }
